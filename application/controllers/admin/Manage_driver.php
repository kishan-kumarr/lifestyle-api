<?php
  ...
  class Manage_driver extends MY_Controller
     {
      ...
      function index()
        {
          $cond=array();
          $order=array('id','DESC');
          $data['driverList'] = $this->DriverModel->getAllDriverDetail();
          $this->load->view('admin/header');
          $this->load->view('admin/sidebar');
          $this->load->view('admin/all_driver' , $data);
       ...
       if(isset($_POST['submit']))
          {
            ...
            $this->form_validation->set_rules('branchManager', 'Branch Manager', 'trim|required');
                                  ...
                                  if($result)
                                    {
                                      $insert_data1  = array(
                                            ...
                                            'under_branch_manager' => $this->input->post('branchManager'),
     ...
     function updateDriver($id = ''){
                                ...
                                if(!$this->upload->do_upload('vehicleImage'))
                                 {
                                    ...
                                    redirect('update-driver/'.$id);
                                 }
                                else
                                  {
                                    $vehicleImage1 = $this->upload->data('file_name');
                                  }
                                }
                                ...
                                if(!$this->upload->do_upload('licenseFrontImage'))
                                 {
                                  ...
                                  {
                                    $licenseFrontImage1 = $this->upload->data('file_name');
                                  }
                                }
                    ...
                    if(empty($_FILES['insuranceFileImage']['name']))
                       ...
                       // ------------------------* End uploading image --------------------------------*
                      ...
                      else
                        {
                         $this->load->view('admin/header');
                         $this->load->view('admin/sidebar');
                         $this->load->view('admin/edit_driver', $driverRecord);
                         $this->load->view('admin/footer');
                        }
      ...
      function changeStatus($id){
        $select= 'status, firstName, lastName, email';
        $cond = array('id' => $id);
        ...
        // ------------------------------- sending mail to driver for on update status ----------------------------
        ...
        $message = $this->load->view('email_templete/statusActiveInactive_template',$mail_data,true);
        
      
        $this->load->library('email');
        $this->email->set_mailtype("html");
        $this->email->set_newline("\r\n");
        $this->email->from($this->lang->line('admin_email'),$this->lang->line('site_name'));
      ...
      function deleteDriver($id){
          ...
          if($this->db->delete($this->table, $cond))
               {
                $condition = array('driverId' => $id);
                $this->db->delete('driverBankDetail', $condition);
                $this->db->delete('driverVehicleDetail', $condition);
                $this->db->delete('driverVehicleLicense', $condition);
     ...
     function viewDriver($id)
      {
       ...
       // print_r($driverRecord['singleRecord']);exit;
         $this->load->view('admin/header');
         $this->load->view('admin/sidebar');
         $this->load->view('admin/view_driver', $driverRecord);
         $this->load->view('admin/footer');
      }
